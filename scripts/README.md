...
Utility scripts for the AntRedo project.

## Available Scripts

### clean-compiled.ps1
Removes all compiled JavaScript files (`.js` and `.js.map`) from `src/` and `test/` directories. These files are generated by TypeScript compilation and should only exist in `dist/`.

**Usage:**
```powershell
.\scripts\clean-compiled.ps1
```

**When to use:**
- After TypeScript compilation errors leave stray `.js` files in source directories
- Before committing to ensure no compiled files are accidentally committed
- When you see `.js` files appearing in `src/` or `test/` folders
